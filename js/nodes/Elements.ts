...
export class Elements extends Array<Element> {
  ...
  get(index: number): Element {
    if (index < 0 || index >= this.length)
      throw new Error(`@index invalid in [0, ${this.length}]`);
    return this[index];
  }
  ...
  val(value?: string): any {
    if (value === undefined)
      return this.length === 0 ? "" : this.first()?.val();
    else {
  ...
  private siblings(query?: string, next?: boolean, all?: boolean): Elements {
    ...
    let evalu: any = !Helper.isNull(query)
   ...
   */
  forms(): FormElement[] {
    return this.filter((el) => Helper.isFormElement(el));
  }
  ...
  private childNodesOfType<T extends Node>(classType: any): T[] {
    let fnc = (childNode: any[]): any[] =>
      childNode.filter((node) => node instanceof classType);
