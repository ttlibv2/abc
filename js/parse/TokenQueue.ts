 ...
 */
 export class TokenQueue {
  ...
  matchesWhitespace(): boolean {
      return !this.isEmpty() && Helper.isWhitespace(this.queue.charAt(this.pos));
  }
   ...
   */
  matchesWord(): boolean {
      return !this.isEmpty() && Helper.isLetterOrDigit(this.queue.charAt(this.pos));
  }
  ...
  consumeWhitespace(): boolean {
      let seen = false;
      while (this.matchesWhitespace()) { this.pos++; seen = true; }
      return seen;
  }
